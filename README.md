...
RTF вырос на длинном файле (ASR 0.084 против 0.031 на 30-минутном), но
пайплайн всё равно **втрое под бюджетом** 0.4. pyannote 1,5 часа держит,
в память не упирается. Почему время не линейно по длине (и это не из-за
видео-предобработки) — в [docs/performance.md](docs/performance.md).
