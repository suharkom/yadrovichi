...
RTF зависит не от длины, а от плотности речи: на семинарах-обсуждениях он
ниже (0.051–0.062), на плотных лекциях с формулами выше (0.103–0.127) —
причём длинный семинар (1 ч 16 мин, 0.051) быстрее по RTF короткого. Везде
**кратно под бюджетом** 0.4, pyannote полтора часа держит и в память не
упирается. Разбор — в [docs/performance.md](docs/performance.md).
...
- WER: на записи с субтитрами, оба текста через нормализацию. Статус, методика и
  промежуточное сравнение с встроенной транскрипцией платформы — в
